...
async def detect_payee_name_from_upi(upi_id: str) -> str:
    """Attempts to auto-detect the registered bank account payee name for a UPI ID."""
    upi_id = upi_id.strip()
    if "@" not in upi_id:
        return ""

    # Strategy 1: Paytm open verify endpoint
    try:
    ...
    # Strategy 2: Upier directory lookup
    try:
    ...
    # Fallback to current saved payee name
...
# ==========================================
# TEMPLATES (LOGIN & STREAMLINED UPI PANEL)
# ==========================================
LOGIN_PAGE = """
<!DOCTYPE html>
<html lang="en">
...
<body class="text-slate-100 min-h-screen flex overflow-x-hidden">
    ...
    <main class="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto">
        ...
        <div class="p-4 md:p-8 max-w-5xl w-full mx-auto space-y-6">
            ...
            <!-- TAB 1: SETTINGS (SIMPLIFIED UPI + AUTO-DETECT + PASSWORD) -->
            <div id="tab-settings" class="tab-content space-y-6 hidden">
                <!-- Direct UPI Setup with Payee Auto-Detect -->
                <form method="POST" action="/admin/settings/upi" class="card-glow rounded-2xl p-6 space-y-5">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-slate-300 mb-2">Payee Registered Name (Auto-detected or custom)</label>
        ...
        async function autoDetectName() {
            ...
            try {
                ...
                if (data.name && data.name.trim().length > 0) {
                    ...
                    statusEl.innerText = "⚠️ Could not auto-detect automatically. You can enter your name manually above.";
                    statusEl.className = "text-[11px] text-amber-400 mt-1.5";
                }
            } catch (err) {
                spinner.classList.add('hidden');
                statusEl.innerText = "⚠️ Lookup service unreachable. Please type your name manually.";
                statusEl.className = "text-[11px] text-amber-400 mt-1.5";
            }
        }
    </script>
...
async def save_upi_setup(
    ...
    # If the user leaves payee_name empty, trigger auto-detect
    if not cleaned_name:
