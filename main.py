        ...
        try:
            ...
            try:
                await manager.bot.send_message(
                    chat_id=admin_id,
                    text=f"{caption}\n\n <i>(Screenshot could not be loaded directly)</i>",
                    parse_mode="HTML",
                    reply_markup=builder.as_markup(),
                )
            except Exception:
...
async def invalid_proof(message: types.Message, state: FSMContext):
    if message.text == "/cancel":
        ...
        await send_welcome_flow(message.chat.id)
        return
    msg = await message.answer(" Please send an image screenshot.")
...
# ==========================================
# TEMPLATES (NAGATO PANEL BRANDING)
# ==========================================
LOGIN_PAGE = """
<!DOCTYPE html>
<html lang="en">
...
<body class="text-slate-100 min-h-screen flex overflow-x-hidden">
    ...
    <!-- 3-Line Sliding Sidebar (Matching Reference Image) -->
